...
private:
  ...
  // ROS
  ros::Subscriber sub_map_{ nh_priv_.subscribe(occupancy_map_topic_, 10, &DijkstraPlanner::initializeMap, this) };
  ros::Subscriber sub_goal_{ nh_priv_.subscribe(goal_topic_, 10, &DijkstraPlanner::wavePropagation, this) };
  ros::Publisher pub_path_{ nh_priv_.advertise<nav_msgs::Path>("path", 10) };

  ros::Timer path_finding_timer_{ nh_priv_.createTimer(path_update_rate_, &DijkstraPlanner::findPath, this, false,
                                                          false) };

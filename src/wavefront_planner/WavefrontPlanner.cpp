...
bool WavefrontPlanner::findWavefrontPath(const Position2D& start_position, const Position2D& goal_position,
                                         std::vector<Position2D>& path, bool search_unknown_area)
{
  ...
  std::tie(path_is_valid, path) = wavefrontAlgorithm(start_index, goal_index, search_unknown_area);
                                                                                    ...
                                                                                    bool search_unknown_area)
{
  ...
  while (!search_list.empty())
  {
    ...
    for (search_iterator; !search_iterator.isPastEnd(); ++search_iterator)
    {
      ...
      if (occupancymap_.isUnknownAt(*search_iterator) && !search_unknown_area)
        continue;
